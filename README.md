# Stock Portfolio Manager v1.4.0
...
- **持股明細**：股數、均價、投入成本、即時現價、預估收入（扣手續費與交易稅）、損益、報酬率；持股可附加備註，顯示於產業 badge 下方
- **多幣別顯示**：右上角 TWD / USD / CNY / THB / JPY 切換，所有金額（均價、市值、損益等）依即時匯率即時換算；匯率每小時自動更新（open.er-api.com）
- **拖拉排序**：持股列可自由拖拉調整順序，自動儲存
- **資產配置**：台股 / A股 / 美股三組圓餅圖分開顯示，各自計算佔比
- **總資產配置**：流動預備金、多筆貸款管理（支援貸款名稱、年利率、還款期數，自動計算月還款與合計）
- **每月支出追蹤**：點擊卡片可直接內嵌編輯每月生活費，自動加總生活費 + 所有貸款月還款；生活費也可在退休規劃設定中修改
- **配息試算**：輸入年化殖利率（或點擊「自動取得」批次從 Yahoo Finance 填入），自動計算稅前/稅後年配息、月配息
- **退休規劃（FIRE）**：計算 FIRE 達成率、需達殖利率、配息缺口，稅換算匯率依選定幣別即時顯示；各組合重點頁面同步顯示進度條
- **個股查詢**：搜尋任意股票，查看 K 線圖（1m～5y）、開高低量、本益比、EPS
- **新增持股自動搜尋**：支援台股/美股（Yahoo Finance）與陸股 A 股（Sina Finance，簡體中文 / 代號 / 拼音）兩種搜尋模式，可在設定頁切換；幣別欄位為分段按鈕，選股後自動帶入
- **深色 / 淺色 / 系統模式**切換
...
| A股 上交所（SSE） | 代號 + `.SS` | `600036.SS`、`600519.SS` |
| A股 深交所（SZSE） | 代號 + `.SZ` | `000001.SZ`、`000977.SZ` |

> **A股搜尋**：請先到設定頁將「股票搜尋市場」切換為「陸股（A股）」，搜尋時支援簡體中文名稱、股票代號與拼音縮寫（如 `zsyh` → 招商银行）。
