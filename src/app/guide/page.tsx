      ...
      <div className="h-full overflow-y-auto p-4">
        <div className="max-w-2xl space-y-4">
          ...
          <Section icon={LayoutDashboard} title="股票配置">
            ...
            <Item label="幣別換算" desc="右上角有 TWD / USD / CNY / THB / JPY 切換按鈕，切換後均價、投入成本、現價、市值、損益等所有金額欄位皆依即時匯率換算顯示，摘要卡片（總市值、總成本、未實現損益）同步換算。" />
            <Item label="欄位說明" desc="「均價」為買入平均成本；「投入成本」= 股數 × 均價；「預估收入」為扣除手續費與交易稅後的估算入帳金額；「備註」顯示在產業 badge 下方。" />
            <Item label="損益 / 報酬率" desc="以「預估收入 − 含買入手續費成本」計算，與券商顯示的損益一致。" />
            <Item label="排序" desc="滑鼠移到持股列，左側出現拖拉把手（⠿），按住拖曳可調整順序，放開後自動儲存。" />
            <Item label="新增持股" desc="點擊右上角「新增持股」，在股票代號欄輸入後會出現搜尋下拉選單，選取後自動帶入名稱與幣別（幣別欄為分段按鈕，也可手動切換）。搜尋範圍由設定頁的「股市切換」決定。" />
            <Item label="備註" desc="新增 / 編輯持股時可填入備註（選填），儲存後顯示於持股列的產業 badge 下方。" />
            <Item label="股市切換" desc="右上角「股市切換」按鈕可快速跳轉到設定頁的股票市場切換區塊。" />
          ...
          <Section icon={PieChart} title="總資產配置">
            ...
            <Item label="配息區塊" desc="對每檔持股輸入預期年化殖利率（%），輸入後 0.6 秒自動儲存，也可按 Tab 移到下一欄立即存入。配息以市值計算，非預估收入。點擊右上角「自動取得」可批次從 Yahoo Finance 抓取殖利率，有資料的自動填入；沒有資料的（如 A 股）保持手動值不變。" />
            <Item label="退休規劃參考" desc="顯示年配息、稅後年 / 月收入、需達殖利率（達到 FIRE 所需的殖利率 %）、配息缺口（每年還差多少配息），以及稅換算匯率（依右上角選定幣別）。點擊右上角「設定」可設定配息稅率、每月生活費目標與手續費率。FIRE 達成率進度條同步顯示於下方。" />
          ...
          <Section icon={TrendingUp} title="殖利率與配息計算方式">
            ...
            <p>需達殖利率 = (每月生活費 × 12 ÷ 稅後比例) ÷ 總市值 × 100%</p>
            <p>配息缺口 = max(0, 需達年配息 − 目前年配息)</p>
            <p className="pt-1 text-muted-foreground/70">殖利率可手動輸入或點擊「自動取得」從 Yahoo Finance 批次填入；Yahoo Finance 無資料的股票（如 A 股）手動值不受影響。</p>
          </Section>
